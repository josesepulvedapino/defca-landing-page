        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
          {attributes.map((attr, index) => (
            ...
            >
              ...
              <div className="relative bg-background border-2 border-primary/20 rounded-3xl p-6 sm:p-8 hover:border-primary/40 transition-colors">
                <motion.div
                  initial={{ rotate: -180, scale: 0 }}
                  animate={isInView ? { rotate: 0, scale: 1 } : {}}
                  transition={{ duration: 0.6, delay: index * 0.15 + 0.3 }}
                  className="w-12 h-12 sm:w-16 sm:h-16 bg-primary rounded-2xl flex items-center justify-center mb-4 sm:mb-6"
                >
                  <attr.icon className="w-6 h-6 sm:w-8 sm:h-8 text-primary-foreground" />
                </motion.div>

                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">{attr.title}</h3>
                <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">{attr.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
